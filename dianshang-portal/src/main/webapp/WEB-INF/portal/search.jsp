...
<div id="J_searchWrap" class="w">
							...
							<div id="J_filter" class="filter">
								<div class="f-line top">
									<div class="f-sort"><a href="javascript:;" class="curr">综合排序<i></i></a><a href="javascript:;" class="">销量<i></i></a>
										...
										<a href="javascript:serachKeyword('${sort}');" class="">价格
											<i></i></a>
					...
					<b>${pageSuperPojos.pageNum}</b><em>/</em><i>${pageSuperPojos.pages}</i>
							...
							<div id="J_goodsList" class="goods-list-v1 gl-type-1 J-goods-list">
								<ul class="gl-warp clearfix" data-tpl="1">
									<c:forEach items="${keyword1.result}" var="product">
										<li data-sku="1711416562" class="gl-item">
											<div class="gl-i-wrap">
												<div class="p-img">
													<a href="javascript:;" onclick="window.open('/product/detail.do?productId=${product.id}')" style="position: relative;">
	...
	<span class="r inb_a page_b">
		<a href="/search.do?keyword=${keyword}&pageNum=1&sort=${sort2}"><font
				size="2">首页</font></a>

		  <c:if test="${keyword1.pageNum<=1}">
			  <font size="2">上一页</font>
		  </c:if>
	    <c:if test="${keyword1.pageNum>1}">
			<a href="/search.do?keyword=${keyword}&pageNum=${keyword1.pageNum-1}&sort=${sort2}"><font size="2">上一页</font></a>
		</c:if>
		 <c:forEach begin="${begin}" end="${end}" var="ps">
			 <c:if test="${keyword1.pageNum==ps}">
				 <strong>${ps}</strong>
			 </c:if>
			 <c:if test="${keyword1.pageNum!=ps}">
				 <a href="/search.do?keyword=${keyword}&pageNum=${ps}&sort=${sort2}">${ps}</a>
			 </c:if>
			 &nbsp;
		 </c:forEach>
		  <c:if test="${keyword1.pageNum>=keyword1.pages}">
			  <font size="2">下一页</font>
		  </c:if>
	    <c:if test="${keyword1.pageNum<keyword1.pages}">
			<a href="/search.do?keyword=${keyword}&pageNum=${keyword1.pageNum+1}&sort=${sort2}"><font size="2">下一页</font></a>
		</c:if>

		<a href="/search.do?keyword=${keyword}&pageNum=${keyword1.pages}&sort=${sort2}"><font
				size="2">尾页</font></a>

		共<var>${keyword1.pages}</var>页 到第<input type="text" size="3" id="PAGENO"/>页 <input type="button" onclick="javascript:window.location.href = '/search.do?keyword=${keyword}&sort=${sort2}&amp;pageNum=' + $('#PAGENO').val() " value="确定" class="hand btn60x20" id="skip"/>


	</span>
								</div>
								<span class="clr">
									1232222223423523523523523523523523523523
								</span>
							</div>
							<div class="page pb15">
